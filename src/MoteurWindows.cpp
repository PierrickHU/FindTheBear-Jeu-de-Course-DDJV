...
namespace PM3D
{
   ...
   float CMoteurWindows::GetTimeIntervalsInSec(int64_t start, int64_t stop) const
   {
   ...
   void CMoteurWindows::BeginRenderSceneSpecific()
   {
      ...
      //CECI EST UN HACK, FAUT LE LAISSER NOIR

      if (gameState == MAIN_MENU)
      {
         float Couleur[4] = { 0.0f, 0.0f, 0.f, 1.0f };  //  RGBA - Black, petit hack pour le probleme de alpha sur une texture au premier plan
         pImmediateContext->ClearRenderTargetView(pRenderTargetView, Couleur);
      }
      else
      {
         float Couleur[4] = { 0.0f, 0.5f, 0.f, 1.0f };  //  RGBA - Vert pour le moment
         pImmediateContext->ClearRenderTargetView(pRenderTargetView, Couleur);
      }
   ...
   void CMoteurWindows::CleanDepthStencil() {
      ID3D11DeviceContext* pImmediateContext = pDispositif->GetImmediateContext();
   ...
   bool CMoteurWindows::RenderScene()
   {
      // Clear le back buffer et le depth buffer du dispositif
      BeginRenderSceneSpecific();

      // Sauvegarde un pointeur vers le back buffer et le depth buffer du dispositifs
      // Liaison du render target et du depth stencil associé au dispositif.
      pPanneauPE->DebutPostEffect();

      // Clear le back buffer et le depth buffer du panneau
      BeginRenderSceneSpecific();

      // on met a jour la vitesse max du joueur pour le panneau de postEffect
      //pPanneauPE->setVitesseMaxJoueur(scene.getVehicule()->input->VITESSE_LINEAIRE_MAX);
      pPanneauPE->setVitesseMaxJoueur(scene.getVehicule()->input->getVitesseLineaireMax());
      // on met a jour la vitesse max du joueur pour le panneau de postEffect
      pPanneauPE->setVitesseJoueur(scene.getVehicule()->input->currentVitesse);
      // Appeler les fonctions de dessin de chaque objet de la scene
      scene.Draw(camera.getFrustrumView(), gameState);
      ...
      {
         for (auto& obj : HUDElements) obj->Draw();
      }
