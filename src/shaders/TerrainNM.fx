...
VS_Sortie MiniPhongVS(float4 Pos : POSITION, float3 Normale : NORMAL, float2 coordTex : TEXCOORD,float3 tan : TANGENT, float3 bitan : BITANGENT)
{
	VS_Sortie sortie = (VS_Sortie)0;

	sortie.Height = Pos.y;
	sortie.DistanceToCenter = sqrt((Pos.x - centreTerrain.x)*(Pos.x - centreTerrain.x) + (Pos.z - centreTerrain.x)*(Pos.z - centreTerrain.x));
	sortie.Pos = mul(Pos, matWorldViewProj);
	sortie.Norm = mul(float4(Normale, 0.0f), matWorld).xyz;
	sortie.tangent = mul(tan, (float3x3)matWorld);
	sortie.tangent = normalize(sortie.tangent);
	sortie.bitangent = mul(bitan, (float3x3)matWorld);
	sortie.bitangent = normalize(sortie.bitangent);
...
float4 MiniPhongPS(VS_Sortie vs) : SV_Target
{
   ...
   if (vs.Height < seuilBas) {
       ...
       normalTexture = normalTexture0.Sample(SampleState, vs.coordTex);
   }
   else if (vs.Height > seuilHaut) {
       couleurTexture = texture1.Sample(SampleState, vs.coordTex).rgb;
       normalTexture = normalTexture1.Sample(SampleState, vs.coordTex);
   }
   else {
       ...
       normalTexture = normalTexture0.Sample(SampleState, vs.coordTex).rgb * (1.f - dx) + normalTexture1.Sample(SampleState, vs.coordTex).rgb * dx;
   }
   // Puis on ajoute la distance au chateau
   float seuilDistanceChateauCourte = 600.f;
   float seuilDistanceChateauLongue = 900.f;
   if (vs.DistanceToCenter < seuilDistanceChateauCourte) {
       couleurTexture = texture2.Sample(SampleState, vs.coordTex).rgb;
       normalTexture = normalTexture2.Sample(SampleState, vs.coordTex).rgb;
   }
       ...
       normalTexture = normalTexture * dx + normalTexture2.Sample(SampleState, vs.coordTex).rgb * (1.0f - dx);
   }
   else {}
   ...
   N = (2 * normalTexture.r - 1) * vs.tangent +
   (2 * normalTexture.g - 1) * vs.bitangent +
   (2 * normalTexture.b - 1) * vs.Norm;
