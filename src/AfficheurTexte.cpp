...
CAfficheurTexte::CAfficheurTexte(CDispositifD3D11* pDispositif, int largeur, int hauteur, Gdiplus::Font* pPolice, const Gdiplus::Color& color)
	: pDispo(pDispositif)
	, TexWidth(largeur)
	, TexHeight(hauteur)
	, pFont(pPolice)
    , size{ static_cast<float>(largeur),static_cast<float>(hauteur) }
	, pTexture(nullptr)
	, pSurface(nullptr)
	...
	, pBlackBrush(nullptr)
{
    ...
    // CEST CORRECT JVAIS LE FAIRE, MERCI POUR TOUT
	pBlackBrush = std::make_unique<Gdiplus::SolidBrush>(color);
...
DirectX::XMFLOAT2& CAfficheurTexte::getSize() {
    return size;
}
