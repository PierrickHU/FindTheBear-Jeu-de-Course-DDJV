...
namespace PM3D
{
   ...
   void CPanneauPE::InitEffet()
   {
      ...
      // Pour l'effet
      pEffet = CMoteurWindows::GetInstance().GetShaderManager().GetEffet(L"../ressources/shaders/PostEffect.fx");

      //****** TECHNIQUE NUL *******
      pTechnique = pEffet->GetTechniqueByName("Nul");
      ...
      //****** TECHNIQUE RADIAL BLUR *******
      pTechnique = pEffet->GetTechniqueByName("RadialBlur");
      pPasse = pTechnique->GetPassByIndex(0);
      ...
      //****** TECHNIQUE GrayScale *******
      pTechnique = pEffet->GetTechniqueByName("GrayScale");
      pPasse = pTechnique->GetPassByIndex(0);
   ...
   void CPanneauPE::Draw(ID3D11ShaderResourceView* pDepthShaderResourceView)
   {
      // Obtenir le contexte
      ...
      auto etat = CMoteurWindows::GetInstance().getGameManager().getGameResult();

      if (etat != GameResult::LOSE) {
         ...
         distance->SetFloat(0.007f * pourcentageVitesse());

         // La «constante» puissanceAttenuation
         GameManager& gm = CMoteurWindows::GetInstance().getGameManager();
         ID3DX11EffectScalarVariable* puissanceAttenuation;
         puissanceAttenuation = pEffet->GetVariableByName("puissanceAttenuation")->AsScalar();
         puissanceAttenuation->SetFloat(gm.shader_getPuissanceAttenuation());

         // La «constante» puissanceEclaircissement
         ID3DX11EffectScalarVariable* puissanceEclaircissement;
         puissanceEclaircissement = pEffet->GetVariableByName("puissanceEclaircissement")->AsScalar();
         puissanceEclaircissement->SetFloat(gm.shader_getPuissanceEclaircissement());
      ...
      pImmediateContext->PSSetShaderResources(0, 1, &nullResource);
      pImmediateContext->PSSetShaderResources(1, 1, &nullResource);
   }

   void CPanneauPE::setVitesseMaxJoueur(float vitesse) {
      vitesseMaxJoueur = vitesse;
   }

   void CPanneauPE::setVitesseJoueur(float vitesse) {
